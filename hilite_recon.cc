...
float min_3(float a,float b,float c){
    float temp = (a>b)?b:a;
    float min_val = (temp>c)?c:temp;
    return min_val;
}
float max_3(float a,float b,float c){
    float temp = (a>b)?a:b;
    float max_val = (temp>c)?temp:c;
    return max_val;
}
...
void HLRecovery_inpaint(float* red_array, float* green_array, float* blue_array, struct imagefloat *imagedata)
{  

  int blur=1;
  float chmax[3]={0};
  float clmax[3]={0};
  struct Imatrices imatrices;
  //  BENCHFUN
    double progress = 0.0;
    ...
    array2D<float> red(width, height,red_array,0); 
    array2D<float> green(width, height,green_array,0); 
    array2D<float> blue(width, height,blue_array,0); 
    ...
    const float minFactor = min_3(factor[0], factor[1], factor[2]);
    ...
    const float whitept = max_3(max_f[0], max_f[1], max_f[2]);
    const float clippt  = min_3(max_f[0], max_f[1], max_f[2]);
    ...
    minx = max(0, minx - blurBorder);
    miny = max(0, miny - blurBorder);
    maxx = min(width - 1, maxx + blurBorder);
    maxy = min(height - 1, maxy + blurBorder);
    ...
    if (blur > 0) {
        ...
        for (int y = 0; y < H2; ++y) {
            for (int x = 0; x < W2; ++x) {
                guide[y][x] = gamma[rgbLuminance(rbuf[y][x], gbuf[y][x], bbuf[y][x], xyzcam)];
            }
        }
    }
//end adding code ART
